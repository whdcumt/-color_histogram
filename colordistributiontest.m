...
%%以上几种方法虽然失败了，但是从中发现了问题的所在就是颜色值必须是一维的，不能像plot3的前三个参数一样，可以矩阵
%所以后面我们修改为逐个点逐个点的画点。这样就保证了这个时候的颜色值是一维数组（RGB三值）
%同时matlab描述RGB三个通道的数值为[0 1]而不是[0 255],需要将图像的原通道值分别除以255转化到该范围。